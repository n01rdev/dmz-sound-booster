...
pub struct DynImmediatePublisher<'a, T: Clone>(
    pub(super) ImmediatePub<'a, dyn PubSubBehavior<T> + 'a, T>,
);
...
impl<'s, 'a, PSB: PubSubBehavior<T> + ?Sized, T: Clone> Future
    for PublisherWaitFuture<'s, 'a, PSB, T>
{
    type Output = ();
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        match self
            .publisher
            .channel
            .publish_with_context(message, Some(cx))
        {
            Ok(()) => Poll::Ready(()),
            Err(message) => {
...
impl<'s, 'a, PSB: PubSubBehavior<T> + ?Sized, T: Clone> Unpin
    for PublisherWaitFuture<'s, 'a, PSB, T>
{
}
