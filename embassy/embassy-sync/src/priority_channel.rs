...
use crate::channel::{
    DynamicChannel, DynamicReceiver, DynamicSender, TryReceiveError, TrySendError,
};
...
mod tests {
    ...
    async fn receiver_receives_given_try_send_async() {
        ...
        static CHANNEL: StaticCell<PriorityChannel<CriticalSectionRawMutex, u32, Max, 3>> =
            StaticCell::new();
    ...
    async fn senders_sends_wait_until_capacity() {
        ...
        static CHANNEL: StaticCell<PriorityChannel<CriticalSectionRawMutex, u32, Max, 1>> =
            StaticCell::new();
        let c = &*CHANNEL.init(PriorityChannel::new());
        assert!(c.try_send(1).is_ok());
