...
impl<'d> MsOsDescriptorWriter<'d> {
    ...
    pub fn header(&mut self, windows_version: u32, vendor_code: u8) {
        assert!(
            self.is_empty(),
            "You can only call MsOsDescriptorWriter::header once"
        );
    ...
    pub fn configuration(&mut self, config: u8) {
        ...
        Self::end_subset::<ConfigurationSubsetHeader>(
            self.buf,
            self.position,
            &mut self.config_mark,
        );
    ...
    fn end(&mut self) {
        if self.position > 0 {
            Self::end_subset::<FunctionSubsetHeader>(
                self.buf,
                self.position,
                &mut self.function_mark,
            );
            Self::end_subset::<ConfigurationSubsetHeader>(
                self.buf,
                self.position,
...
impl<'a> PropertyData<'a> {
    ...
    pub fn size(&self) -> usize {
        match self {
            ...
            PropertyData::DwordLittleEndian(val) | PropertyData::DwordBigEndian(val) => {
