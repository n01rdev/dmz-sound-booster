    ...
    E: NorFlashError,
{
    ...
    async fn read(&mut self, mut offset: u32, mut bytes: &mut [u8]) -> Result<(), E> {
        ...
        if !bytes.is_empty() {
            self.1
                .read(offset - self.0.capacity() as u32, bytes)
                .await?;
        }
    ...
    E: NorFlashError,
{
    ...
    async fn write(&mut self, mut offset: u32, mut bytes: &[u8]) -> Result<(), E> {
        ...
        if !bytes.is_empty() {
            self.1
                .write(offset - self.0.capacity() as u32, bytes)
                .await?;
        }
...
mod tests {
    ...
    fn can_write_and_read_across_flashes() {
        ...
        f.write(60, &[0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88])
            .unwrap();
