...
mod alt_regions {
    ...
    use crate::_generated::flash_regions::{
        OTPRegion, BANK1_REGION1, BANK1_REGION2, BANK1_REGION3, OTP_REGION,
    };
    use crate::flash::{
        asynch, Async, Bank1Region1, Bank1Region2, Blocking, Error, Flash, FlashBank, FlashRegion,
    };
    ...
    pub struct AltBank1Region3<'d, MODE = Async>(
        pub &'static FlashRegion,
        PeripheralRef<'d, FLASH>,
        PhantomData<MODE>,
    );
    pub struct AltBank2Region1<'d, MODE = Async>(
        pub &'static FlashRegion,
        PeripheralRef<'d, FLASH>,
        PhantomData<MODE>,
    );
    pub struct AltBank2Region2<'d, MODE = Async>(
        pub &'static FlashRegion,
        PeripheralRef<'d, FLASH>,
        PhantomData<MODE>,
    );
    pub struct AltBank2Region3<'d, MODE = Async>(
        pub &'static FlashRegion,
        PeripheralRef<'d, FLASH>,
        PhantomData<MODE>,
    );
    ...
    impl<'d> Flash<'d> {
        pub fn into_alt_regions(self) -> AltFlashLayout<'d, Async> {
            ...
            AltFlashLayout {
                ...
                bank1_region3: AltBank1Region3(
                    &ALT_BANK1_REGION3,
                    unsafe { p.clone_unchecked() },
                    PhantomData,
                ),
                bank2_region1: AltBank2Region1(
                    &ALT_BANK2_REGION1,
                    unsafe { p.clone_unchecked() },
                    PhantomData,
                ),
                bank2_region2: AltBank2Region2(
                    &ALT_BANK2_REGION2,
                    unsafe { p.clone_unchecked() },
                    PhantomData,
                ),
                bank2_region3: AltBank2Region3(
                    &ALT_BANK2_REGION3,
                    unsafe { p.clone_unchecked() },
                    PhantomData,
                ),
...
pub(crate) fn assert_not_corrupted_read(end_address: u32) {
    ...
    #[allow(unused)]
    let second_bank_read = get_flash_regions().last().unwrap().bank == FlashBank::Bank2
        && end_address > (FLASH_SIZE / 2) as u32;
...
mod tests {
    ...
    fn can_get_sector() {
        ...
        let assert_sector =
            |snb: u8, bank: FlashBank, index_in_bank: u8, start: u32, size: u32, address: u32| {
