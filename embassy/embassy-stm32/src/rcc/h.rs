...
pub use crate::pac::rcc::vals::{
    Ckpersel as PerClockSource, Fdcansel as FdCanClockSource, Hsidiv as HSIPrescaler,
    Plldiv as PllDiv, Pllm as PllPreDiv, Plln as PllMul, Pllsrc as PllSource, Sw as Sysclk,
};
...
pub(crate) unsafe fn init(config: Config) {
        ...
        match config.supply_config {
            ...
            | SupplyConfig::SMPSExternalLDOBypass(ref smps_supply_voltage) => {
                PWR.cr3().modify(|w| {
                    ...
                    w.set_bypass(matches!(
                        config.supply_config,
                        SupplyConfig::SMPSExternalLDOBypass(_)
                    ));
                });
            }
    ...
    RCC.cfgr()
        .modify(|w| w.set_timpre(config.timer_prescaler.into()));
...
fn init_pll(num: usize, config: Option<Pll>, input: &PllInput) -> PllOutput {
    let Some(config) = config else {
        ...
        #[cfg(stm32h7)]
        RCC.pllckselr()
            .write(|w| w.set_divm(num, PllPreDiv::from_bits(0)));
        #[cfg(stm32h5)]
        RCC.pllcfgr(num)
            .write(|w| w.set_divm(PllPreDiv::from_bits(0)));
