...
fn calc_ns_per_timer_tick<T: Instance>(mode: config::FrameTransmissionConfig) -> u64 {
    match mode {
        ...
        config::FrameTransmissionConfig::ClassicCanOnly => {
            ...
            let prescale: u64 = ({ T::regs().nbtp().read().nbrp() } + 1) as u64
                * ({ T::regs().tscc().read().tcp() } + 1) as u64;
        ...
        impl<'d, T: Instance> Fdcan<'d, T, fdcan::$from_mode> {
            ...
            pub fn $name(self) -> Fdcan<'d, T, fdcan::$to_mode> {
                let ns_per_timer_tick =
                    calc_ns_per_timer_tick::<T>(self.can.get_config().frame_transmit);
                Fdcan {
                    can: self.can.$func(),
                    ns_per_timer_tick,
                }
    ...
    M: fdcan::Receive,
{
    ...
    pub async fn write(&mut self, frame: &TxFrame) -> Option<TxFrame> {
        poll_fn(|cx| {
            ...
            if let Ok(dropped) = self.can.transmit_preserve(
                frame.header,
                &frame.data.bytes,
