...
impl<'a, W: Word> ReadableDmaRingBuffer<'a, W> {
    ...
    pub fn read(
        ...
        dma: &mut impl DmaCtrl,
        buf: &mut [W],
    ) -> Result<(usize, usize), OverrunError> {
        /*
        ...
        if self.start == end && dma.get_complete_count() == 0 {
            ...
            */
            let (pos, complete_count) =
                critical_section::with(|_| (self.pos(dma), dma.get_complete_count()));
            if (pos >= self.start && pos < end)
                || (complete_count > 0 && pos >= end)
                || complete_count > 1
            {
                Err(OverrunError)
            } else {
...
impl<'a, W: Word> WritableDmaRingBuffer<'a, W> {
    ...
    pub fn write(
        ...
        if start > self.end {
            ...
            let (pos, complete_count) =
                critical_section::with(|_| (self.pos(dma), dma.get_complete_count()));
            if (pos >= self.end && pos < start)
                || (complete_count > 0 && pos >= start)
                || complete_count > 1
            {
                Err(OverrunError)
            } else {
