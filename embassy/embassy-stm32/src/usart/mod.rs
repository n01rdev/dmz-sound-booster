...
impl<'d, T: BasicInstance, TxDma> UartTx<'d, T, TxDma> {
        ...
        TxDma: crate::usart::TxDma<T>,
    {
        ...
        let transfer =
            unsafe { Transfer::new_write(ch, request, buffer, tdr(T::regs()), Default::default()) };
        transfer.await;
        Ok(())
    }
...
impl<'d, T: BasicInstance, RxDma> UartRx<'d, T, RxDma> {
        ...
        RxDma: crate::usart::RxDma<T>,
    {
        ...
        let transfer =
            unsafe { Transfer::new_read(ch, request, rdr(T::regs()), buffer, Default::default()) };
...
impl<'d, T: BasicInstance, TxDma> embedded_hal_02::blocking::serial::Write<u8>
    for UartTx<'d, T, TxDma>
{
    type Error = Error;
...
impl<'d, T: BasicInstance, TxDma, RxDma> embedded_hal_02::serial::Read<u8>
...
impl<'d, T: BasicInstance, TxDma, RxDma> embedded_hal_nb::serial::ErrorType
    for Uart<'d, T, TxDma, RxDma>
{
    type Error = Error;
}
...
impl<'d, T: BasicInstance, TxDma, RxDma> embedded_hal_nb::serial::Read
    for Uart<'d, T, TxDma, RxDma>
{
    fn read(&mut self) -> Result<u8, nb::Error<Self::Error>> {
        self.nb_read()
...
impl<'d, T: BasicInstance, TxDma, RxDma> embedded_hal_nb::serial::Write
    for Uart<'d, T, TxDma, RxDma>
{
    fn write(&mut self, char: u8) -> nb::Result<(), Self::Error> {
        self.blocking_write(&[char]).map_err(nb::Error::Other)
