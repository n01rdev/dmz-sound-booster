...
impl Ipcc {
    ...
    pub async fn send(channel: IpccChannel, f: impl FnOnce()) {
        ...
        regs.cpu(0)
            .scr()
            .write(|w| w.set_chs(channel as usize, true));
    }
    ...
    pub async fn flush(channel: IpccChannel) {
        ...
        poll_fn(|cx| {
            ...
            regs.cpu(0)
                .mr()
                .modify(|w| w.set_chfm(channel as usize, false));
            ...
            if !regs.cpu(0).sr().read().chf(channel as usize) {
                ...
                regs.cpu(0)
                    .mr()
                    .modify(|w| w.set_chfm(channel as usize, true));
    ...
    pub async fn receive<R>(channel: IpccChannel, mut f: impl FnMut() -> Option<R>) -> R {
        ...
        loop {
            ...
            poll_fn(|cx| {
                ...
                regs.cpu(0)
                    .mr()
                    .modify(|w| w.set_chom(channel as usize, false));
            ...
            regs.cpu(0)
                .scr()
                .write(|w| w.set_chc(channel as usize, true));
        }
    }
}
