...
fn main() {
    ...
    if !time_driver_singleton.is_empty() {
        println!(
            ...
            time_driver_singleton.to_lowercase()
        );
    }
    ...
    let rcc_enum_map: HashMap<&str, HashMap<&str, &Enum>> = {
        let rcc_blocks = rcc_registers
            ...
            .unwrap()
            .items;
        let rcc_fieldsets: HashMap<&str, &FieldSet> = rcc_registers
            .ir
            .fieldsets
            .iter()
            .map(|f| (f.name, f))
            .collect();
        let rcc_enums: HashMap<&str, &Enum> =
            rcc_registers.ir.enums.iter().map(|e| (e.name, e)).collect();
    ...
    for p in METADATA.peripherals {
        ...
        if let Some(rcc) = &p.rcc {
            ...
            let ptype = if let Some(reg) = &p.registers {
                reg.kind
            } else {
                ""
            };
            ...
            let refcount = force_refcount.contains(ptype)
                || *rcc_field_count.get(&(en.register, en.field)).unwrap() > 1;
            let (before_enable, before_disable) = if refcount {
                let refcount_static = format_ident!(
                    "{}_{}",
                    en.register.to_ascii_uppercase(),
                    en.field.to_ascii_uppercase()
                );
