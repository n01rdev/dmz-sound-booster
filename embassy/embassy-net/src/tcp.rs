...
impl<'d> TcpIo<'d> {
    ...
    async fn flush(&mut self) -> Result<(), Error> {
        poll_fn(move |cx| {
            self.with_mut(|s, _| {
                let waiting_close =
                    s.state() == tcp::State::Closed && s.remote_endpoint().is_some();
        ...
        embedded_nal_async::TcpConnect for TcpClient<'d, D, N, TX_SZ, RX_SZ>
    {
        ...
        async fn connect<'a>(
            ...
            let addr: crate::IpAddress = match remote.ip() {
                ...
                IpAddr::V4(addr) => {
                    crate::IpAddress::Ipv4(crate::Ipv4Address::from_bytes(&addr.octets()))
                }
                #[cfg(not(feature = "proto-ipv4"))]
                IpAddr::V4(_) => panic!("ipv4 support not enabled"),
                #[cfg(feature = "proto-ipv6")]
                IpAddr::V6(addr) => {
                    crate::IpAddress::Ipv6(crate::Ipv6Address::from_bytes(&addr.octets()))
                }
